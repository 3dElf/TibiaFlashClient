package mx.controls
{
   ...
   public class List extends ListBase implements IIMESupport
   {
      ...
      override protected function mouseUpHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(_loc3_) && Boolean(_loc3_.data) && _loc3_ != itemEditorInstance)
         {
            _loc7_ = itemRendererToIndices(_loc3_);
            if(Boolean(editable) && !dontEdit)
            {
      ...
      private function editingTemporarilyPrevented(param1:Object) : Boolean
      {
         var _loc2_:int = 0;
         var _loc3_:IListItemRenderer = null;
         if(Boolean(runningDataEffect) && Boolean(param1))
         {
      ...
      override protected function makeRowsAndColumns(param1:Number, param2:Number, param3:Number, param4:Number, param5:int, param6:int, param7:Boolean = false, param8:uint = 0) : Point
      {
         ...
         yy = top;
         rowNum = firstRow;
         more = iterator != null && !iterator.afterLast && Boolean(iteratorValid);
         while(!byCount && yy < bottom || Boolean(byCount) && rowsNeeded > 0)
         {
            if(byCount)
            {
               rowsNeeded--;
            ...
            if(valid)
            {
               ...
               if(!item || (Boolean(runningDataEffect) && Boolean(dataItemWrappersByRenderer[item])?Boolean(dataItemWrappersByRenderer[item] != wrappedData):Boolean(item.data != data)))
               {
               ...
               item.explicitWidth = ww;
               if(item is IInvalidating && (Boolean(wordWrapChanged) || Boolean(variableRowHeight)))
               {
            ...
            if(wrappedData != data)
            {
               bSelected = Boolean(bSelected) || Boolean(selectedData[itemToUID(data)]);
               bSelected = Boolean(bSelected) && !getRendererSemanticValue(item,ModifiedCollectionView.REPLACEMENT) && !getRendererSemanticValue(item,ModifiedCollectionView.ADDED);
            }
            ...
            rowNum++;
            rowsMade++;
            if(Boolean(iterator) && Boolean(more))
            {
               try
               {
                  more = iterator.moveNext();
      ...
      override protected function measure() : void
      {
         ...
         if(Boolean(initialized) && Boolean(variableRowHeight) && explicitRowCount < 1 && Boolean(isNaN(explicitRowHeight)))
         {
      ...
      override protected function focusInHandler(param1:FocusEvent) : void
      {
         ...
         if(Boolean(editable) && !isPressed)
         {
            ...
            if(!_editedItemPosition)
            {
               ...
               _loc2_ = Boolean(Boolean(listItems.length) && listItems[0].length > 0);
            }
            if(_loc2_)
            {
      ...
      private function commitEditedItemPosition(param1:Object) : void
      {
         ...
         if(Boolean(itemEditorInstance && param1) && Boolean(itemEditorInstance is IFocusManagerComponent) && _editedItemPosition.rowIndex == param1.rowIndex)
         {
         ...
         if(_loc2_ > _loc4_)
         {
            ...
            {
               while(_loc2_ > _loc5_ || Boolean(_loc2_ == _loc5_ && _loc2_ > verticalScrollPosition + _loc4_) && Boolean(_loc6_))
               {
      ...
      {
         if(param1 is MouseEvent && Boolean(itemRendererContains(itemEditorInstance,DisplayObject(param1.target))))
         {
            return;
         }
         endEdit(ListEventReason.OTHER);
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(Boolean(bEditedItemPositionChanged) && !editingTemporarilyPrevented(_proposedEditedItemPosition))
         {
            bEditedItemPositionChanged = false;
            commitEditedItemPosition(_proposedEditedItemPosition);
            _proposedEditedItemPosition = undefined;
