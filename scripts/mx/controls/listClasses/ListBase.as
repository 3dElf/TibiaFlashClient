package mx.controls.listClasses
{
   ...
   public class ListBase extends ScrollControlBase implements IDataRenderer, IFocusManagerComponent, mx.controls.listClasses.IListItemRenderer, IDropInListItemRenderer, IEffectTargetHost
   {
      ...
      protected function mouseMoveHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(isPressed) && Boolean(mouseDownPoint) && (Math.abs(mouseDownPoint.x - _loc2_.x) > DRAG_THRESHOLD || Math.abs(mouseDownPoint.y - _loc2_.y) > DRAG_THRESHOLD))
         {
            if(Boolean(dragEnabled) && !DragManager.isDragging && Boolean(mouseDownPoint))
            {
         ...
         if(Boolean(_loc3_) && Boolean(highlightItemRenderer))
         {
            ...
            if(Boolean(highlightItemRenderer) && Boolean(highlightUID) && _loc5_.uid != highlightUID)
            {
               if(!isPressed)
               {
      ...
      protected function dragEnterHandler(param1:DragEvent) : void
      {
         ...
         if(Boolean(enabled) && Boolean(iteratorValid) && Boolean(param1.dragSource.hasFormat("items")))
         {
      ...
      override public function set horizontalScrollPosition(param1:Number) : void
      {
         ...
         if(_loc2_ != param1)
         {
            ...
            if(Boolean(bColumnScrolling) && _loc3_ >= columnCount)
            {
               clearIndicators();
               clearVisibleData();
      ...
      protected function updateList() : void
      {
         ...
         if(iterator)
         {
            if(Boolean(int(offscreenExtraColumns)) || Boolean(int(offscreenExtraColumnsLeft)) || Boolean(offscreenExtraColumnsRight))
            {
      ...
      protected function dragScroll() : void
      {
         ...
         if(Boolean(DragManager.isDragging) && Boolean(lastDragEvent) && _loc3_ != verticalScrollPosition)
         {
      ...
      protected function calculateDropIndicatorY(param1:Number, param2:int) : Number
      {
         var _loc3_:int = 0;
         var _loc4_:Number = 0;
         if(Boolean(param1) && Boolean(param2 < param1) && Boolean(listItems[param2].length) && Boolean(listItems[param2][0]))
         {
      ...
      override protected function commitProperties() : void
      {
         ...
         if(columnCountChanged)
         {
            ...
            if(!isNaN(explicitWidth) && Boolean(isNaN(explicitColumnWidth)) && explicitColumnCount > 0)
            {
         ...
         if(rowCountChanged)
         {
            ...
            if(!isNaN(explicitHeight) && Boolean(isNaN(explicitRowHeight)) && explicitRowCount > 0)
            {
      ...
      protected function findKey(param1:int) : Boolean
      {
         ...
         return _loc2_ >= 33 && _loc2_ <= 126 && Boolean(findString(String.fromCharCode(_loc2_)));
      }
      ...
      private function adjustSelectionSettings(param1:Boolean) : void
      {
         if(bSelectionChanged)
         {
            ...
            if(Boolean(bSelectedIndicesChanged) && (Boolean(param1) || _selectedIndices == null))
            {
               bSelectedIndicesChanged = false;
               bSelectedIndexChanged = false;
               commitSelectedIndices(_selectedIndices);
            }
            if(Boolean(bSelectedItemChanged) && (Boolean(param1) || _selectedItem == null))
            {
               bSelectedItemChanged = false;
               bSelectedIndexChanged = false;
               commitSelectedItem(_selectedItem);
            }
            if(Boolean(bSelectedItemsChanged) && (Boolean(param1) || _selectedItems == null))
            {
               bSelectedItemsChanged = false;
               bSelectedIndexChanged = false;
               commitSelectedItems(_selectedItems);
            }
            if(Boolean(bSelectedIndexChanged) && (Boolean(param1) || _selectedIndex == -1))
            {
      ...
      protected function mouseOverHandler(param1:MouseEvent) : void
      {
         ...
         if(!isPressed || Boolean(allowDragSelection))
         {
            if(param1.relatedObject)
            {
               ...
               if(Boolean(itemRendererContains(_loc3_,param1.relatedObject)) || _loc5_ == _loc6_ || param1.relatedObject == highlightIndicator)
               {
                  return;
               }
            }
         ...
         else
         {
            ...
            if(dragScrollingInterval != 0 && Boolean(allowDragSelection) || Boolean(menuSelectionMode))
            {
      ...
      protected function finishKeySelection() : void
      {
         ...
         if(Boolean(bSelectItem) && caretIndex - verticalScrollPosition >= 0)
         {
      ...
      protected function selectItem(param1:mx.controls.listClasses.IListItemRenderer, param2:Boolean, param3:Boolean, param4:Boolean = true) : Boolean
      {
         ...
         if(!allowMultipleSelection || !shiftKey && !ctrlKey)
         {
            ...
            if(Boolean(ctrlKey) && Boolean(selectedData[uid]))
            {
               selectionChange = true;
               clearSelected(transition);
            }
            else if(_selectedIndex != index || Boolean(bSelectedIndexChanged) || Boolean(allowMultipleSelection) && numSelected != 1)
            {
               selectionChange = true;
               clearSelected(transition);
         ...
         else if(Boolean(shiftKey) && Boolean(allowMultipleSelection))
         {
            if(anchorBookmark)
            {
         ...
         else if(Boolean(ctrlKey) && Boolean(allowMultipleSelection))
         {
            if(selectedData[uid])
            {
      ...
      protected function prepareDataEffect(param1:CollectionEvent) : void
      {
         ...
         if(Boolean(cachedItemsChangeEffect) && Boolean(iteratorValid))
         {
      ...
      protected function mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(dragEnabled) && Boolean(selectedData[rowMap[_loc2_.name].uid]))
         {
            bSelectOnRelease = true;
         }
      ...
      protected function drawItem(param1:mx.controls.listClasses.IListItemRenderer, param2:Boolean = false, param3:Boolean = false, param4:Boolean = false, param5:Boolean = false) : void
      {
         ...
         if(Boolean(param3) && (!highlightItemRenderer || highlightUID != _loc11_.uid))
         {
            if(!highlightIndicator)
            {
         ...
         else if(Boolean(!param3) && Boolean(highlightItemRenderer) && (Boolean(_loc11_) && Boolean(highlightUID == _loc11_.uid)))
         {
         ...
         else if(Boolean(!param4) && Boolean(caretItemRenderer) && caretUID == _loc11_.uid)
         {
      ...
      private function makeRowsAndColumnsWithExtraRows(param1:Number, param2:Number) : void
      {
         ...
         if(_loc9_ > 0 && !iterator.afterLast)
         {
            ...
            if(_loc7_.y == _loc9_)
            {
               while(Boolean(_loc7_.y > 0) && Boolean(listItems[listItems.length - 1]) && listItems[listItems.length - 1].length == 0)
               {
                  _loc7_.y--;
                  listItems.pop();
                  rowInfo.pop();
      ...
      protected function mouseOutHandler(param1:MouseEvent) : void
      {
         ...
         if(!isPressed)
         {
            if(Boolean(itemRendererContains(_loc2_,param1.relatedObject)) || param1.relatedObject == listContent || param1.relatedObject == highlightIndicator || !highlightItemRenderer)
            {
               return;
            }
      ...
      {
         if(Boolean(_dropEnabled) && !param1)
         {
      ...
      {
         if(Boolean(_dragEnabled) && !param1)
         {
      ...
      protected function collectionChangeHandler(param1:Event) : void
      {
         ...
         if(event is CollectionEvent)
         {
            ...
            {
               if(collection.length == 0 || Boolean(runningDataEffect) && actualCollection.length == 0)
               {
               ...
               if(Boolean(bSelectedIndexChanged) || Boolean(bSelectedItemChanged) || Boolean(bSelectedIndicesChanged) || Boolean(bSelectedItemsChanged))
               {
                  bSelectionChanged = true;
               }
               else
      ...
      protected function dragDropHandler(param1:DragEvent) : void
      {
         ...
         lastDragEvent = null;
         resetDragScrolling();
         if(Boolean(enabled) && Boolean(param1.dragSource.hasFormat("items")))
         {
            if(!dataProvider)
            {
               dataProvider = [];
            ...
            if(param1.action == DragManager.MOVE && Boolean(dragMoveEnabled))
            {
      ...
      {
         if(Boolean(runningDataEffect) || Boolean(runDataEffectNextUpdate))
         {
            return;
         }
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         adjustListContent(param1,param2);
         var _loc5_:Boolean = Boolean(collection) && collection.length > 0;
         if(_loc5_)
         {
            adjustScrollPosition();
         }
         if(Boolean(oldUnscaledWidth == param1 && !scrollAreaChanged && !itemsSizeChanged && listItems.length > 0) && Boolean(iterator) && columnCount == 1)
         {
         ...
         if(Boolean(keySelectionPending) && Boolean(iteratorValid))
         {
