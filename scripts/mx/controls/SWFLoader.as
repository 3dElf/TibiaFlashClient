package mx.controls
{
   ...
   public class SWFLoader extends UIComponent implements ISWFLoader
   {
      ...
      private function doScaleContent() : void
      {
         ...
         if(!resizableContent || Boolean(maintainAspectRatio) && !flexContent)
         {
            unScaleContent();
         ...
         else
         {
            ...
            if(contentHolder is Loader)
            {
               ...
               try
               {
                  ...
                  else if(Boolean(childAllowsParent) && !(holder.content is IFlexDisplayObject))
                  {
                     contentHolder.width = w;
                     contentHolder.height = h;
                  }
      ...
      {
         var _loc1_:Number = Boolean(isNaN(_bytesTotal)) || _bytesTotal == 0?Number(0):Number(100 * (_bytesLoaded / _bytesTotal));
         if(isNaN(_loc1_))
         {
            _loc1_ = 0;
         }
      ...
      private function contentLoaded() : void
      {
         ...
         try
         {
            if(Boolean(tabChildren) && contentHolder is Loader && (loaderInfo.contentType == "application/x-shockwave-flash" || Loader(contentHolder).content is DisplayObjectContainer))
            {
         ...
         if(contentHolder)
         {
            if(isContentLoaded)
            {
               if(contentHolder is Loader)
               {
                  ...
                  if(Boolean(childAllowsParent) && content == SystemManager.lastSystemManager)
                  {
                     SystemManager.lastSystemManager = null;
                  }
                  removeInitSystemManagerCompleteListener(Loader(contentHolder).contentLoaderInfo);
                  if(Boolean(useUnloadAndStop) && "unloadAndStop" in contentHolder)
                  {
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(isContentLoaded)
         {
            if(Boolean(_scaleContent) && !brokenImage)
            {
               doScaleContent();
            }
            else
         ...
         if(Boolean(brokenImage) && !brokenImageBorder)
         {
      ...
      private function mouseShieldHandler(param1:Event) : void
      {
         ...
         if(!isContentLoaded || Boolean(parentAllowsChild))
         {
            return;
         }
         if(param1["value"])
