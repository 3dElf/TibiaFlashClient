package shared.utility
{
   ...
   public class SharedObjectManager
   {
      ...
      protected static function s_AllocateSpaceInternal(param1:int, param2:Function, param3:Boolean = true) : void
      {
         ...
         else if(FlushStatus == SharedObjectFlushStatus.PENDING)
         {
            Listener = function(param1:NetStatusEvent):void
            {
               ...
               if(Boolean(a_FirstPass) && a_RequestedSize >= STORAGE_UNLIMITED && !checkPlayerVersion(10,1,0,0))
               {
      ...
      {
         return param1 != null && param1.length > 0 && this.m_SharedObject != null && Boolean(this.m_SharedObject.data.hasOwnProperty(param1));
      }
      ...
      public function clear(param1:String, param2:Boolean = false) : void
      {
         ...
         {
            if(!this.exists(a_Name) || Boolean(!a_IncludeProtected) && Boolean(this.m_SharedObject.data[a_Name].isProtected))
            {
               return;
            }
         }
      ...
      public function getListing(param1:Boolean = false) : Array
      {
         ...
         if(this.m_SharedObject != null)
         {
            ...
            {
               if(!this.m_SharedObject.data[_loc3_].isProtected || Boolean(param1))
               {
                  _loc2_.push(_loc3_);
               }
            }
