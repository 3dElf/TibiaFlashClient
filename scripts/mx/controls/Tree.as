package mx.controls
{
   ...
   public class Tree extends List implements IIMESupport
   {
      ...
      override protected function dragOverHandler(param1:DragEvent) : void
      {
         ...
         try
         {
            if(Boolean(iteratorValid) && Boolean(event.dragSource.hasFormat("treeItems")))
            {
      ...
      private function updateDropData(param1:DragEvent) : void
      {
         ...
         if(_loc7_ > _loc8_ * 0.5 && Boolean(isItemOpen(_loc14_)) && Boolean(_dataDescriptor.isBranch(_loc14_,iterator.view)) && (!_dataDescriptor.hasChildren(_loc14_,iterator.view) || _dataDescriptor.getChildren(_loc14_,iterator.view).length == 0))
         {
            _loc9_ = _loc14_;
            _loc10_ = 0;
            _loc11_ = true;
      ...
      override public function isItemVisible(param1:Object) : Boolean
      {
         ...
         if(_loc2_)
         {
            ...
            if(Boolean(visibleData[_loc3_]) && Boolean(_openItems[_loc3_]))
            {
               return true;
            }
         }
      ...
      private function getChildIndexInParent(param1:Object, param2:Object) : int
      {
         ...
         else if(param1 != null && Boolean(_dataDescriptor.isBranch(param1,iterator.view)) && Boolean(_dataDescriptor.hasChildren(param1,iterator.view)))
         {
      ...
      override protected function adjustAfterRemove(param1:Array, param2:int, param3:Boolean) : Boolean
      {
         ...
         if(Boolean(bSelectedItemRemoved) && _loc4_ < 1)
         {
      ...
      private function getVisibleChildrenCount(param1:Object) : int
      {
         ...
         if(Boolean(_openItems[_loc3_]) && Boolean(_dataDescriptor.isBranch(param1,iterator.view)) && Boolean(_dataDescriptor.hasChildren(param1,iterator.view)))
         {
      ...
      public function expandChildrenOf(param1:Object, param2:Boolean) : void
      {
         ...
         if(isBranch(param1))
         {
            ...
            if(param1 != null && Boolean(_dataDescriptor.isBranch(param1,iterator.view)) && Boolean(_dataDescriptor.hasChildren(param1,iterator.view)))
            {
      ...
      override public function itemToIcon(param1:Object) : Class
      {
         ...
         var iconClass:Class = Boolean(itemIcons) && Boolean(itemIcons[uid])?itemIcons[uid][!!open?"iconID2":"iconID"]:null;
         if(iconClass)
         {
            return iconClass;
         }
      ...
      override mx_internal function selectionDataPendingResultHandler(param1:Object, param2:ListBaseSelectionDataPending) : void
      {
         ...
         if(Boolean(bFinishArrowKeySelection) && selectedItem === proposedSelectedItem)
         {
            finishArrowKeySelection();
         }
      }
      ...
      {
         if(param1 == null || param1 == "styleName" || Boolean(IS_NEW_ROW_STYLE[param1]))
         {
            itemsSizeChanged = true;
            invalidateDisplayList();
         }
      ...
      public function expandItem(param1:Object, param2:Boolean, param3:Boolean = false, param4:Boolean = false, param5:Event = null) : void
      {
         ...
         var uid:String = itemToUID(item);
         if(!isBranch(item) || isItemOpen(item) == open || Boolean(isOpening))
         {
            return;
         }
         if(itemEditorInstance)
         ...
         if(isItemVisible(item))
         {
            ...
            if(Boolean(animate) && rowIndex < rC && rowsToMove > 0 && rowsToMove < 20 && dur != 0)
            {
               if(tween)
               {
                  tween.endTween();
               ...
               if(open)
               {
                  ...
                  while(i < rowsToMove && yy < maxDist)
                  {
                     ...
                     if(renderer is IInvalidating && (Boolean(wordWrapChanged) || Boolean(variableRowHeight)))
                     {
      ...
      override protected function commitProperties() : void
      {
         ...
         if(Boolean(dataProviderChanged) || Boolean(showRootChanged) || Boolean(openItemsChanged))
         {
      ...
      private function getOpenChildrenStack(param1:Object, param2:Array) : Array
      {
         ...
         while(!_loc5_.afterLast)
         {
            ...
            param2.push(_loc3_);
            if(Boolean(isBranch(_loc3_)) && Boolean(isItemOpen(_loc3_)))
            {
      ...
      private function checkItemIndices(param1:DragEvent) : void
      {
         ...
         if((param1.action == DragManager.MOVE || param1.action == DragManager.NONE) && Boolean(dragMoveEnabled))
         {
            if(param1.dragInitiator == this)
            {
               _loc2_ = param1.dragSource.dataForFormat("treeItems") as Array;
      ...
      override protected function dragCompleteHandler(param1:DragEvent) : void
      {
         ...
         try
         {
            ...
            {
               if(event.action == DragManager.MOVE && Boolean(dragMoveEnabled))
               {
