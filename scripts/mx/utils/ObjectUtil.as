   ...
   public class ObjectUtil
   {
      ...
      public static function getClassInfo(param1:Object, param2:Array = null, param3:Object = null) : Object
      {
         ...
         else if(dynamic)
         {
            ...
            numericIndex = Boolean(isArray) && !isNaN(Number(p));
         }
         if(!(Boolean(isArray) || Boolean(isDict) || className == "Object"))
         {
            if(className == "XML")
            {
               n = properties.length();
            ...
            else
            {
               ...
               while(i < n)
               {
                  ...
                  if(excludeObject[p] != 1)
                  {
                     if(!(!options.includeTransient && Boolean(internalHasMetadata(metadataInfo,p,"Transient"))))
                     {
                        if(uris != null)
                        {
