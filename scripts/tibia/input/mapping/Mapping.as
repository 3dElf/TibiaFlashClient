package tibia.input.mapping
{
   ...
   public class Mapping
   {
      ...
      function onKeyInput(param1:uint, param2:uint, param3:uint, param4:Boolean, param5:Boolean, param6:Boolean) : void
      {
         ...
         {
            if(Boolean(a_Alt) && (Boolean(a_Ctrl) || Boolean(a_Shift)) && Boolean(b.appliesTo(a_Event,a_Key,true,false,false)) || Boolean(a_Ctrl) && (Boolean(a_Alt) || Boolean(a_Shift)) && Boolean(b.appliesTo(a_Event,a_Key,false,true,false)) || Boolean(a_Shift) && (Boolean(a_Alt) || Boolean(a_Ctrl)) && Boolean(b.appliesTo(a_Event,a_Key,false,false,true)) || Boolean(a_Alt) && Boolean(a_Ctrl) && Boolean(b.appliesTo(a_Event,a_Key,false,false,false)) || Boolean(a_Alt) && Boolean(a_Shift) && Boolean(b.appliesTo(a_Event,a_Key,false,false,false)) || Boolean(a_Ctrl) && Boolean(a_Shift) && Boolean(b.appliesTo(a_Event,a_Key,false,false,false)) || Boolean(b.appliesTo(a_Event,a_Key,a_Alt,a_Ctrl,a_Shift)))
            {
               try
               {
                  _Action = b.action;
      ...
      public function marshall(param1:Boolean = true) : XML
      {
         ...
         while(_loc4_ < _loc3_)
         {
            if(this.m_Binding[_loc4_] != null && (!param1 || Boolean(this.m_Binding[_loc4_].editable)))
            {
      ...
      private function addItemInternal(param1:tibia.input.mapping.Binding) : Boolean
      {
         ...
         while(_loc2_ >= 0)
         {
            ...
            if(Boolean(this.m_Binding[_loc2_].action.equals(param1.action)) && ++_loc3_ > 2)
            {
               return false;
            }
            _loc2_--;
      ...
      public function removeAll(param1:Boolean = true) : void
      {
         ...
         else
         {
            ...
            while(_loc3_ >= 0)
            {
               if(this.m_Binding[_loc3_] == null || Boolean(this.m_Binding[_loc3_].editable))
               {
                  _loc2_--;
