   ...
   public class MiniMapStorage extends EventDispatcher
   {
         ...
         if(a_Event != null)
         {
            if(this.m_IEQueue != null && this.m_IEQueue.length > 0)
            {
               ...
               try
               {
                  ...
                  _SharedObject = null;
                  _MiniMapSector = null;
                  if(Boolean(SharedObjectManager.s_SharedObjectsAvailable()) && (_SharedObjectManager = SharedObjectManager.s_GetInstance()) != null && (_SharedObject = _SharedObjectManager.getLocal(Name,false,true)) != null && (_MiniMapSector = tibia.minimap.MiniMapSector.s_LoadSharedObject(_SharedObject)) != null)
                  {
      ...
      public function calculatePath(param1:int, param2:int, param3:int, param4:int, param5:int, param6:int, param7:Boolean, param8:Boolean, param9:Array) : int
      {
         ...
         if(Math.abs(_loc10_) + Math.abs(_loc11_) == 1)
         {
            ...
            if(Boolean(param8) || _loc12_ < PATH_COST_OBSTACLE)
            {
         ...
         if(Boolean(param7) && Math.abs(_loc10_) == 1 && Math.abs(_loc11_) == 1)
         {
            _loc12_ = this.getFieldCost(param4,param5,param6);
            if(Boolean(param8) || _loc12_ < PATH_COST_OBSTACLE)
            {
