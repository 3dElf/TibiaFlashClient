package mx.binding
{
   ...
   public class Binding
   {
      ...
      public function execute(param1:Object = null) : void
      {
         ...
         if(Boolean(isExecuting) || Boolean(twoWayCounterpart) && Boolean(twoWayCounterpart.isExecuting))
         {
            hasHadValue = true;
            return;
         }
      ...
      private function innerExecute() : void
      {
         ...
         if(Boolean(hasHadValue) || Boolean(wrappedFunctionSuccessful))
         {
            if(!(lastValue is XML && Boolean(lastValue.hasComplexContent()) && lastValue === _loc1_) && !(Boolean(lastValue is XMLList && lastValue.hasComplexContent()) && _loc1_ is XMLList && nodeSeqEqual(lastValue as XMLList,_loc1_ as XMLList)))
            {
