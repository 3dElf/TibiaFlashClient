   ...
   public class DataGridHeader extends DataGridHeaderBase
   {
      ...
      protected function mouseUpHandler(param1:MouseEvent) : void
      {
         ...
         else
         {
            ...
            {
               if(param1.target == separators[_loc6_] && Boolean(visibleColumns[_loc6_].resizable))
               {
                  return;
               }
               _loc6_++;
         ...
         if(Boolean(dataGrid.enabled && (dataGrid.sortableColumns || dataGrid.draggableColumns)) && Boolean(dataGrid.dataProvider) && Boolean(dataGrid.headerVisible))
         {
            if(_loc3_ == lastItemDown)
            {
               if(Boolean(dataGrid.sortableColumns) && Boolean(visibleColumns[_loc6_].sortable))
               {
                  lastItemDown = null;
      ...
      protected function drawSeparators() : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            ...
            if(!(Boolean(_loc6_) && _loc6_.length > 0 || dataGrid.headerVisible))
            {
      ...
      protected function mouseOutHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(resizeGraphic) || Boolean(dataGrid.movingColumn))
         {
            return;
         }
         if(Boolean(dataGrid.enabled) && Boolean(dataGrid.sortableColumns) && Boolean(dataGrid.headerVisible))
         {
            ...
            else
            {
               ...
               {
                  if(param1.target == separators[_loc3_] && Boolean(visibleColumns[_loc3_].resizable))
                  {
                     return;
                  }
                  _loc3_++;
            ...
            if(visibleColumns.length > 0 && Boolean(visibleColumns[_loc3_].sortable))
            {
      ...
      protected function placeSortArrow() : void
      {
         ...
         if(Boolean(visibleColumns.length) && Boolean(dataGrid.lastSortIndex >= 0) && dataGrid.lastSortIndex != dataGrid.sortIndex)
         {
         ...
         if(Boolean(_loc5_) && sortArrow.x < headerItems[_loc4_].x)
         {
            sortArrow.visible = false;
         }
         if(!sortArrow.visible && Boolean(_loc1_))
         {
            _loc1_.visible = false;
         }
      }
      ...
      protected function mouseOverHandler(param1:MouseEvent) : void
      {
         ...
         var _loc5_:Point = null;
         var _loc6_:Sprite = null;
         if(Boolean(resizingColumn) || Boolean(dataGrid.movingColumn))
         {
            return;
         }
         if(Boolean(dataGrid.enabled) && Boolean(dataGrid.sortableColumns) && Boolean(dataGrid.headerVisible))
      ...
      protected function mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         else
         {
            ...
            {
               if(param1.target == separators[_loc4_] && Boolean(visibleColumns[_loc4_].resizable))
               {
                  return;
               }
               _loc4_++;
         ...
         if(Boolean(dataGrid.enabled) && (Boolean(dataGrid.sortableColumns) || Boolean(dataGrid.draggableColumns)) && Boolean(dataGrid.headerVisible))
         {
            if(Boolean(dataGrid.sortableColumns) && Boolean(visibleColumns[_loc4_].sortable))
            {
               lastItemDown = _loc2_;
            ...
            if(Boolean(dataGrid.draggableColumns) && Boolean(visibleColumns[_loc4_].draggable))
            {
               startX = NaN;
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(Boolean(headerItemsChanged) && (Boolean(_loc3_ && _loc3_.length > 0) || Boolean(dataGrid.headerVisible)))
         {
            ...
            while(_loc19_ < _loc18_.length)
            {
               ...
               if(Boolean(_loc12_) && headerItems.indexOf(_loc12_) == -1)
               {
                  if(_loc12_.parent == this)
                  {
