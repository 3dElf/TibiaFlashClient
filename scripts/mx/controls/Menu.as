   ...
   public class Menu extends List implements IFocusManagerContainer
   {
      ...
      private function moveSelBy(param1:Number, param2:Number) : void
      {
         ...
         while(true)
         {
            ...
            if(!(Boolean(_loc6_.data) && (_dataDescriptor.getType(_loc6_.data) == "separator" || !_dataDescriptor.isEnabled(_loc6_.data))))
            {
               if(selectedIndex != -1)
               {
            ...
            while(_loc8_ < 2)
            {
               ...
               while(param2)
               {
                  ...
                  if(_loc12_ is IMenuItemRenderer)
                  {
                     ...
                     if(_loc13_.measuredIconWidth > 0 && Boolean(_loc13_.measuredTypeIconWidth))
                     {
                        useTwoColumns = true;
                        _loc9_ = true;
                     }
               ...
               if(_loc8_ == 0)
               {
                  if(!(Boolean(_loc9_) && (maxMeasuredIconWidth + maxMeasuredTypeIconWidth > _loc4_ || maxMeasuredBranchIconWidth > _loc5_)))
                  {
                     break;
                  }
               }
      ...
      override protected function mouseUpHandler(param1:MouseEvent) : void
      {
         ...
         if(_loc4_ != null && Boolean(_dataDescriptor.isEnabled(_loc4_)) && !_dataDescriptor.isBranch(_loc4_))
         {
      ...
      override protected function mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(_loc3_) && Boolean(_dataDescriptor.isEnabled(_loc3_)) && !_dataDescriptor.isBranch(_loc3_))
         {
      ...
      override protected function mouseEventToItemRenderer(param1:MouseEvent) : IListItemRenderer
      {
         ...
         if(Boolean(_loc2_) && Boolean(_loc2_.data) && _dataDescriptor.getType(_loc2_.data) == "separator")
         {
            return null;
         }
         return _loc2_;
      ...
      override protected function keyDownHandler(param1:KeyboardEvent) : void
      {
         ...
         {
            if(Boolean(_loc3_ && _dataDescriptor.isBranch(_loc3_)) && Boolean(_loc4_) && Boolean(_loc4_.visible))
            {
               supposedToLoseFocus = true;
               _loc4_.setFocus();
         ...
         {
            if(Boolean(_loc3_) && Boolean(_dataDescriptor.isBranch(_loc3_)))
            {
               openSubMenu(_loc2_);
      ...
      private function getRowIndex(param1:IListItemRenderer) : int
      {
         ...
         while(_loc2_ < listItems.length)
         {
            ...
            if(Boolean(_loc3_) && Boolean(_loc3_.data) && _dataDescriptor.getType(_loc3_.data) != "separator")
            {
               if(_loc3_ == param1)
               {
                  return _loc2_;
      ...
      override protected function mouseOutHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(itemRendererContains(_loc2_,param1.relatedObject)) || Boolean(itemRendererContains(_loc2_,DisplayObject(param1.target))) || param1.relatedObject == highlightIndicator || param1.relatedObject == listContent || !highlightItemRenderer)
         {
            return;
         }
         if(Boolean(getStyle("useRollOver")) && Boolean(_loc3_))
         {
      ...
      override protected function commitProperties() : void
      {
         ...
         if(Boolean(dataProviderChanged) || Boolean(showRootChanged))
         {
            dataProviderChanged = false;
            showRootChanged = false;
            if(Boolean(_rootModel) && Boolean(!_showRoot) && Boolean(_hasRoot))
            {
               _loc2_ = _rootModel.createCursor().current;
               if(_loc2_ != null && Boolean(_dataDescriptor.isBranch(_loc2_,_rootModel)) && Boolean(_dataDescriptor.hasChildren(_loc2_,_rootModel)))
               {
      ...
      override protected function mouseOverHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(row) && Boolean(row.data))
         {
            item = row.data;
         }
         isPressed = event.buttonDown;
         if(Boolean(row) && row != anchorRow)
         {
            if(anchorRow)
            {
               drawItem(anchorRow,false,false);
         ...
         else if(Boolean(subMenu) && Boolean(subMenu.subMenu))
         {
            subMenu.subMenu.hide();
         }
         if(Boolean(_dataDescriptor.isBranch(item)) && Boolean(_dataDescriptor.isEnabled(item)))
         {
            anchorRow = row;
            if(Boolean(subMenu) && Boolean(subMenu.closeTimer))
            {
         ...
         if(Boolean(item) && Boolean(_dataDescriptor.isEnabled(item)))
         {
            if(event.relatedObject)
            {
               if(Boolean(itemRendererContains(row,event.relatedObject)) || row == lastHighlightItemRenderer || event.relatedObject == highlightIndicator)
               {
                  return;
               }
            }
      ...
      public function show(param1:Object = null, param2:Object = null) : void
      {
         ...
         if(Boolean(parentMenu) && !parentMenu.visible)
         {
            return;
         }
         if(visible)
         ...
         if(Boolean(parentDisplayObject) && (!parent || !parent.contains(parentDisplayObject)))
         {
