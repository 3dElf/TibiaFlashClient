package mx.controls
{
   ...
   public class Button extends UIComponent implements IDataRenderer, IDropInListItemRenderer, IFocusManagerComponent, IListItemRenderer, IFontContextComponent, IButton
   {
      ...
      private function previousVersion_measure() : void
      {
         ...
         if(Boolean(currentSkin) && (Boolean(isNaN(skinMeasuredWidth)) || Boolean(isNaN(skinMeasuredHeight))))
         {
      ...
      mx_internal function viewIconForPhase(param1:String) : IFlexDisplayObject
      {
         ...
         if(!_loc2_)
         {
            ...
            if(!checkedDefaultIcon && Boolean(_loc2_))
            {
         ...
         currentIcon = _loc3_;
         if(Boolean(defaultIconUsesStates) && currentIcon is IStateClient)
         {
            _loc5_ = "";
            if(!enabled)
            {
      ...
      mx_internal function viewSkinForPhase(param1:String, param2:String) : void
      {
         ...
         if(!_loc3_)
         {
            ...
            if(!checkedDefaultSkin && Boolean(_loc3_))
            {
         ...
         if(!_loc4_)
         {
            if(_loc3_)
            {
               ...
               if(_loc4_ is IInvalidating && Boolean(initialized))
               {
                  IInvalidating(_loc4_).validateNow();
               }
               else if(_loc4_ is IProgrammaticSkin && Boolean(initialized))
               {
         ...
         currentSkin = _loc4_;
         if(Boolean(defaultSkinUsesStates) && currentSkin is IStateClient)
         {
      ...
      mx_internal function layoutContents(param1:Number, param2:Number, param3:Boolean) : void
      {
         ...
         var _loc25_:EdgeMetrics = Boolean(currentSkin) && Boolean(currentSkin is IBorder) && !(currentSkin is IFlexAsset)?IBorder(currentSkin).borderMetrics:null;
         if(_loc25_)
         {
      ...
      override protected function measure() : void
      {
         ...
         var _loc8_:EdgeMetrics = Boolean(currentSkin) && Boolean(currentSkin is IBorder) && !(currentSkin is IFlexAsset)?IBorder(currentSkin).borderMetrics:null;
         if(_loc8_)
         {
      ...
      mx_internal function changeSkins() : void
      {
         ...
         if(Boolean(initialized) && FlexVersion.compatibilityVersion >= FlexVersion.VERSION_3_0)
         {
            viewSkin();
            invalidateSize();
         }
      ...
      private function previousVersion_layoutContents(param1:Number, param2:Number, param3:Boolean) : void
      {
         ...
         var _loc21_:Number = !!param3?Number(buttonOffset):Number(0);
         var _loc22_:String = getStyle("textAlign");
         var _loc23_:EdgeMetrics = Boolean(currentSkin) && currentSkin is IRectangularBorder?IRectangularBorder(currentSkin).borderMetrics:null;
         ...
         else
         {
            ...
            if(labelPlacement == ButtonLabelPlacement.TOP)
            {
               ...
               if(Boolean(centerContent) && _loc7_ > 0)
               {
                  _loc7_ = _loc7_ / 2;
               }
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(oldUnscaledWidth > param1 || textField.text != label || Boolean(labelChanged) || Boolean(styleChangedFlag))
         {
